...
export class EmployeeComponent {
  ...
  loadTable(query: string) {
    ...
    this.es.getAll(query)
      .then((emps: Employee[]) => {
        this.employees = emps;
        this.ns.setLastSequenceNumber(this.employees[this.employees.length-1].number);
        this.generateNumber();
  ...
  generateNumber(){
    const newNumber = this.ns.generateNumber('EMP');
    this.form.controls['number'].setValue(newNumber);
  }
  // gensupnumber():void{
  //   this.form.get("supplier")?.valueChanges.subscribe((sup:Supplier)=>{
  //     this.es.getMaxNumber().then(maxnumber=>{
  //       if(maxnumber==null){
  ...
  // }
  // genempnumber():void{
  //   this.form.get("employee")?.valueChanges.subscribe((sup:Employee)=>{
  //     this.es.getMaxNumber().then(maxnumber=>{
  //       if(maxnumber==null){
  //         this.form.get("number")?.setValue("EO0001")
  //       }else {
  //         let s1 = JSON.stringify(maxnumber).toString();
  //         let match1 = s1.match(/\d+/);
  //         // @ts-ignore
  //         let match = parseInt(match1[0], 10);
  //         this.form.get("number")?.setValue("EO0000" + ++match);
  //       }
  //     })
  //   })
  // }
